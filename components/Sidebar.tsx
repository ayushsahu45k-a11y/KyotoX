...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`fixed lg:static inset-y-0 left-0 z-30 w-80 bg-white dark:bg-slate-900 
      border-r border-slate-200 dark:border-slate-800 transform transition-transform duration-300 
      ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0 lg:w-0 lg:overflow-hidden lg:border-none'} 
    ...
    >
      <div className="h-full flex flex-col relative">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-slate-500 dark:text-slate-400 
          hover:text-indigo-600 lg:hidden z-50"
        >
          <XIcon />
        </button>
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          <div
            className={`absolute inset-0 transition-opacity duration-300 ${
              activeTab === 'history' ? 'opacity-100 z-10' : 'opacity-0 z-0'
            }`}
          >
            <div className="h-full flex flex-col p-4 overflow-y-auto custom-scrollbar">
              <button
                onClick={onNewChat}
                className="w-full mb-6 py-3 px-4 rounded-xl border-2 border-dashed 
                border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400 
                hover:border-indigo-500 hover:text-indigo-500 dark:hover:border-indigo-400 
                dark:hover:text-indigo-400 font-semibold text-sm transition-all flex items-center justify-center gap-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
          ...
          <div
            className={`absolute inset-0 transition-opacity duration-300 ${
              activeTab === 'data' ? 'opacity-100 z-10' : 'opacity-0 z-0'
            }`}
          >
            <DatasetVisualizer />
          </div>
            ...
            <div className="relative">
              {user.avatar ? (
                <img
                  ...
                  border-slate-300 dark:border-slate-600 group-hover:border-indigo-500 transition-colors"
                />
              ) : (
